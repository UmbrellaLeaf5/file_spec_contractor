...
def resolve_prompt_path(
    project_root: Path, cfg: dict, cli_prompt: str | None = None
) -> Path | None:
    if cli_prompt:
        p = Path(cli_prompt)
        if p.is_absolute():
    ...
    prompt_rel = cfg.get("prompt", {}).get("file")
    if prompt_rel:
...
def load_prompt(path: Path | None) -> str:
    ...
    return builtin_prompt_text()


def builtin_prompt_text() -> str:
    """Возвращает встроенный промпт из пакета."""
    try:
