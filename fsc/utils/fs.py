...
def resolve_output_path(src_path: Path, project_root: Path, cfg: dict) -> Path:
    mode = cfg.get("output", {}).get("output_mode", "mirror")
    if mode == "adjacent":
        return src_path.with_name(src_path.stem + ".fsc.md")
    out_dir = Path(cfg.get("output", {}).get("output_dir", ".fsc/specs"))
